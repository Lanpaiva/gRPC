...
func (c *CategoryService) GetCategory(ctx context.Context, in *pb.GetCategoryRequest) (*pb.Category, error) {
	...
	if err != nil {
		fmt.Printf("Error to get Category: %v", err)
	}
	categoryResponse := &pb.Category{
		Id:          category.ID,
		Name:        category.Name,
...
func (c *CategoryService) CreateCategoryStreamBidirect(stream pb.CategoryService_CreateCategoryStreamBidirectServer) error {
	for {
		category, err := stream.Recv()
		if err == io.EOF {
			return nil
